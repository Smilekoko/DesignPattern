 ...
 */
class TemplateMethodActivity : AppCompatActivity(), View.OnClickListener {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        val binding = DataBindingUtil.setContentView<ActivityTemplateMethodBinding>(this, R.layout.activity_template_method)
        setTitle("模板方法模式")
    ...
    override fun onClick(v: View) {
        when (v.getId()) {
            R.id.bt_template_method_text -> {
                val itWorker = ITWorker("景彬")
                itWorker.workOneDay()
                val hrWorker = HRWorker("莉莉姐")
                hrWorker.workOneDay()
                val qaWorker = QAWorker("张元元")
                qaWorker.workOneDay()
                val qaWorker1 = QAWorker("徐晨星")
                qaWorker1.workOneDay()
                val ctoWorker = CTOWorker("远哥")
                ctoWorker.workOneDay()
                val otherWorker = OtherWorker("那个谁,就是你")
                otherWorker.workOneDay()
            }

            R.id.bt_template_method_text2 -> {
                val itWorker2 = ITWorker("景彬")
