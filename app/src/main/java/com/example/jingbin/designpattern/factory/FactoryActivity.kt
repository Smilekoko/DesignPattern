 ...
 */
class FactoryActivity : AppCompatActivity(), View.OnClickListener {
    private var binding: ActivityRoujiaMoStoreBinding? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = DataBindingUtil.setContentView<ActivityRoujiaMoStoreBinding>(this, R.layout.activity_roujia_mo_store)
        setTitle("工厂模式")
    ...
    override fun onClick(v: View) {
        when (v.getId()) {
            R.id.bt_static_factory -> Toast.makeText(this, "TextUtil.isEmpty等，类+静态方法.", Toast.LENGTH_SHORT).show()
            R.id.bt_simple_factory -> {
                // 通过专门定义一个类来负责创建其他类的实例，被创建的实例通常都具有共同的父类。
                val roujiaMoStore = RoujiaMoStore(SimpleRoujiaMoFactory())
                roujiaMoStore.sellRoujiaMo("Suan")
                roujiaMoStore.sellRoujiaMo("Tian")
                roujiaMoStore.sellRoujiaMo("La")
            }

            R.id.bt_factory_method -> {
                // 定义：定义一个创建对象的接口，但由子类决定要实例化的类是哪一个。工厂方法模式把类实例化的过程推迟到子类。
                val xianRoujiaMoStore = XianRoujiaMoStore(XianSimpleRoujiaMoFactory()) // 分店简单工厂
                xianRoujiaMoStore.sellRoujiaMo("Suan")
                xianRoujiaMoStore.sellRoujiaMo("Tian")
                xianRoujiaMoStore.sellRoujiaMo("La")
            }

            R.id.bt_abstract_factory -> {
                // 定义：提供一个接口，用于创建相关的或依赖对象的家族，而不需要明确指定具体类。
                val xianRoujiaMoTeSeStore = XianRoujiaMoTeSeStore(XianSimpleRoujiaMoTeSeFactory())
